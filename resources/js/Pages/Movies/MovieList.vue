...
<template>
    <ul class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <li v-for="movie in this.movies" :key="movie.id" class="group col-span-1 grid grid-cols-4 bg-white rounded-lg overflow-hidden shadow divide-y divide-gray-200 transition-all hover:bg-red-50 hover:shadow-lg cursor-pointer">
            ...
            <img class="col-span-1 min-h-full bg-gray-300 flex-shrink-0 group-hover:bg-red-500" v-if="movie.image" :src="movie.image" alt="" />
        </li>
  </ul>
</template>
