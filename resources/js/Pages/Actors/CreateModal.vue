            ...
            <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-y-scroll shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full sm:p-6">
                <div class="flex items-start">
                    <div class="mt-3 text-center sm:mt-0 sm:text-left w-full flex flex-col gap-4">
                        ...
                        <!-- Form -->
                        <form class="rounded-xl px-2 py-4 w-full grid grid-cols-1 gap-8" @submit.prevent="createActor">

                            <div class="col-span-1 flex flex-col gap-3">
                                <!-- Name * -->
                                <div class="flex flex-col gap-1 px-2">
                                    <label for="title">Name</label>
                                    <input name="title" type="text" required v-model="actor.name" autocomplete="off" class="rounded-md" :class="error.name ? 'bg-red-100' : 'bg-white'" placeholder="Actor name...">
                                ...
                                <div class="flex flex-col gap-1 px-2">
                                    ...
                                    <input name="dob" type="date" v-model="actor.dob" autocomplete="off" class="rounded-md" :class="error.dob ? 'bg-red-100' : 'bg-white'" required>
                                ...
                                <div class="flex flex-col gap-1 px-2">
                                    ...
                                    <input type="text" name="image_url" v-model="actor.image_url" class="rounded-md" :class="error.image_url ? 'bg-red-100' : 'bg-white'" autocomplete="off" required placeholder="Actor Image...">
...
export default {
    ...
    methods: {
        ...
        createActor: function() {
            axios.post(`/api/actors`, this.actor)
                .then(() => {
                    this.$emit('createdActor');
                })
                .catch(e => {
